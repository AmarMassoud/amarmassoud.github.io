document.addEventListener("DOMContentLoaded", async () => {
  ...
  const deleteDeal = (purchaseId) => {
    ...
    if (purchaseIndex !== -1) {
      ...
      const dealToDeleteIndex = purchaseToDelete.deals.findIndex(
        (deal) => deal.seller.id === currentUser.id
      );
      ...
      if (dealToDeleteIndex !== -1) {
        ...
        const refundRequest = refundRequests.find(
          (request) => request.purchaseId === purchaseId
        );
        console.log("purchaseId", refundRequest.purchaseId);

        const itemIndex = dealToDelete.items.findIndex(
          (item) => item.product.id === refundRequest.productId
        );
  ...
  const renderRefundPopup = (refundReuqest) => {
    ...
    refundPopUp.className =
      "self-center justify-self-center modal bg-white rounded-lg p-4 flex flex-col  p-6 max-w-[50rem] max-h-[800px] overflow-y-scroll";
    refundPopUp.replaceChildren();

    const closeButton = document.createElement("button");
    closeButton.className =
      "text-custom-red float-right self-end hover:text-red-700";
    ...
    const selectedItemPrice =
      refundReuqest.cartItem.quantity * refundReuqest.cartItem.product?.price;
    priceValue.textContent = selectedItemPrice
      ? `$${selectedItemPrice}`
      : "N/A";
    ...
    selectReasonDiv.className =
      "flex flex-col space-y-2 w-full items-start my-3 ";
    ...
    selectReasonOptions.className =
      "select select-bordered w-full bg-custom-gray text-black";
    ...
    inform.textContent =
      "Please do not include any sensitive information. Information provided in this field will be shared with the asset provider.";

    const messageInput = document.createElement("textarea");
    messageInput.disabled = true;
    messageInput.className =
      "textarea bg-custom-gray textarea-bordered w-full h-32";
    ...
    acceptRejectButtonsDiv.className =
      "flex flex-row space-x-2 w-full justify-end my-3 ";

    const acceptButton = document.createElement("button");
    acceptButton.className =
      "btn btn-primary mt-12 mr-4 hover:bg-green-600 hover:text-white self-end";
    ...
    acceptButton.addEventListener("click", () => {
      ...
      refundRequests = refundRequests.filter(
        (request) => request.id !== refundReuqest.id
      );
    ...
    rejectButton.className =
      "btn btn-primary mt-12 hover:bg-red-600 hover:text-white self-end";
  ...
  const renderRequest = (refundRequest) => {
    ...
    moreDetails.className =
      "text-bs font-semibold text-black-500 bg-white py-2 px-10 rounded-xl hover:text-white  hover:bg-custom-red mt-5 text-nowrap ";
    moreDetails.textContent = "More Details";
    moreDetails.addEventListener("click", () => {
      renderRefundPopup(refundRequest);
    });
